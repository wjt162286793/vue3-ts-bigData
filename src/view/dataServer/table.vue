  ...
  <div class="tableBox">
    <el-table :data="tableData" style="width: 100%" stripe :border="true">
      ...
      <el-table-column prop="domain" label="应用领域" width="300">
        <template #default="scope">
          <el-tag
            v-for="(item, index) in getDomain(scope)"
            :key="index"
      ...
      <el-table-column label="Operations" width="180">
        <template #default="scope">
          <el-button link type="primary" size="small" @click="getDetail(scope)"
            >修改</el-button
          >
...
const getSource = (record: any) => {
  let row = sourceOptions.value.find(
    (item: any) => item.value === record.row.source
  );
  return row?.label;
};
const getDomain = (record: any) => {
  let arr = domainOptions.value.filter((item: any) =>
    record.row.domain.includes(item.value)
  );
  return arr;
};
const getStatus = (record: any) => {
  let row = statusOptions.value.find(
    (item: any) => item.value === record.row.status
  );
  return row?.label;
};
